...
def gauss_elimination(A, b):
    ...
    for i_row in range(n_row):
        ...
        for j_column in range(n_column):
            Ab_row[j_column] = A[i_row][j_column]
        Ab_row[n_column] = b[i_row]
        Ab.append(Ab_row)

    # pivot 반복문
    for i_pivot in range(n_row):
        # pivot 아래 행 반복문
        for j_row in range(i_pivot + 1, n_row):
            ratio = -Ab[j_row][i_pivot] / float(Ab[i_pivot][i_pivot])
            # 열 반복문
            for k_column in range(n_column + 1):
    ...
    for i_row in range(n_row - 1, -1, -1):
        ...
        for k_column in range(i_row + 1, n_row):
